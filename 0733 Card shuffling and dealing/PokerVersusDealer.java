/*
 * Modify the application developed in Exercise 7.32 so that it can handle the dealer's hand automatically, but the player is allowed to decide which cards of the player's hand to replace.  The application should then evaluate both hands and determine who wins.  Now use this new application to play 20 games against the computer.  Who wins more games, you or the computer?  Have a friend play 20 games against the computer.  Who wins more games?  Based on the results of these games, refine your poker player application.  (This, too, is a difficult problem.)  Play 20 more games.  Does your modified application play a better game?
 * 
 * 
 * 
 * 
 * I believe I have already given the discardDraw function the best possible rules to play by, so it will always win a greater number of games against a human player unless that player plays by the same rules.  I feel the real strategy in poker is the sizing up of your human opponent and in trying to decide what his hand might be.  You can't get that with a computer player that follows a basic algorithm.
 * It would obviously be much better to have the player enter in the index of every card they want to discard in one line, however that would require parsing techniques not yet introduced.
 */
import java.util.Scanner;

public class PokerVersusDealer{
