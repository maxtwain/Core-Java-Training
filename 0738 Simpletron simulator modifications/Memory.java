...
public class Memory{
	...
	// SIZE determines both the number of available registers as well as the maximum size of operands.
	...
	void execute() {
		for (instructionCounter = 0; memory[instructionCounter].getOpCode() != 43; ++instructionCounter) {
			...
			case 31:
				...
				break;
			case 32:
				if (memory[operand].getRegister() == 0) {
			...
			case 34:
				...
				break;
			case 35:
				for (int aCopy = accumulator; operand > 1; --operand) {
					accumulator *= aCopy;
					if (accumulator >= SIZE * 100 || accumulator <= SIZE * -100) {
						System.out.println("*** Accumulator Overflow ****");
						System.out.println("*** Simpletron execution abnormally terminated ***");
	...
	public void dump() {
		...
		System.out.printf("%-21s  %03d%n%n", "operand", memory[i].getOperand());
		...
		for (int row = 0; row < SIZE; row += 10) {
			...
			for (int col = 0; col < 10; ++col) {
				System.out.printf("%+06d ", memory[row + col].getRegister());
			}
			System.out.println();
		}
	}
